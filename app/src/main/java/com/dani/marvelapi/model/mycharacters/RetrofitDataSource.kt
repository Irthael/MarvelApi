...
class RetrofitDataSource(private val marvelServiceManager: APIServiceManager): RemoteDataSource {
    ...
    override suspend fun getCharacterFilterList(offset: Int, name: String): List<Mycharacter> {
        val itemsResponse = marvelServiceManager.service.getAllCharactersFilter(offset = offset, name = name)
        return itemsResponse.body()?.data!!.results.toList()
    }
